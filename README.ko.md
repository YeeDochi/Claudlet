...
펫은 이제 듣기만 하지 않아요. **우클릭 → 💬 지금 물어보기** 를 고르면 한 줄 물어볼
창이 뜨고, 친 말이 그대로 세션 프롬프트에 들어가 제출돼요. **📝 쪽지 남기기** 는 펫이
쪽지를 **입에 물고 있다가** 다음 훅 경계에서 전해요 — 에이전트가 일하는 중이면 다음
툴 호출에서 바로 닿고, 쉬고 있으면 다음 프롬프트에 실려 가요. 뭘 물고 있는지 항상
보이니까 모르는 새에 딸려 가는 일은 없고, 마음이 바뀌면 메뉴에서 버리면 돼요.
...
Claude Code 와 Codex 둘 다 돼요. 다만 **터미널에서 돌고 있을 때만** 이에요 —
**지금 물어보기**는 프롬프트에 글자를 직접 넣는 일이라서요. KDE 에서는 Konsole
설정의 *보안에 민감한 DBus API 활성화* 가 켜져 있어야 메뉴에 뜨고, 그게 아닌
곳에서는 쪽지만 보여요.

> **데스크톱 앱에서는 안 돼요.** Claude 데스크톱 앱과 Codex 앱에는 글자를 밀어
> 넣을 프롬프트가 없어요 — 이 기능은 터미널에서 도는 CLI 를 전제로 해요.
> macOS 는 터미널이어도 **지금 물어보기**가 아직 없고 쪽지만 돼요.
