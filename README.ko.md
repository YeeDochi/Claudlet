...
pip install PyQt6
~/claude-pet/bin/claude-pet-install-hooks      # ~/.claude/settings.json 에 훅 등록
~/claude-pet/bin/claude-pet                     # 실행
```

돌아가던 Claude Code 세션이 있으면 재시작해야 훅을 인식해요.

## `/claude-pet` 스킬 (선택)

세션 안에서 원할 때 펫을 띄우는 Claude Code 스킬이에요. 스킬 디렉터리에 링크해서 활성화:
...
이후 아무 세션에서 `/claude-pet`(또는 "펫 띄워")로 실행. 이건 펫을 *띄우기만* 하고, 세션별
자동 실행은 위 훅이 담당해요.
