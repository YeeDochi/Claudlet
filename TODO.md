...
- [x] **포커스 대상 콘솔 혼선** — `_activate_claude`가 첫 클래스 매칭 대신 **호스트 창(internalId)
      ...
      _알려진 한계(WONTFIX, 2026-07-10 합의):_ pid로 창을 찾는데 **konsole은 단일 프로세스**라
      같은 konsole의 여러 창/분리된 탭은 pid가 같아 구분 불가(첫 창을 집음). 별개 앱/프로세스
      사이에선 정확. 정밀화하려면 `KONSOLE_DBUS_*` 환경변수(konsole 전용, 탭분리 시 id 변함)
      뿐이라 투자 대비 애매 → 그대로 두기로.
...
부분가림 클리핑·완전가림 숨김·raise 즉시반영·작업표시줄 안뜸). session-bound 펫 2마리 독립 반응·SessionEnd 퇴장(라이브),
