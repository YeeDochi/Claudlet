...
- **Windows**: nothing extra — window integration uses only the stdlib `ctypes`
  bindings to `user32`/`dwmapi`/`kernel32` in `app/src/claudlet/platform/geom/win32.py`.
- **macOS only**: `pip install pyobjc-framework-Quartz` for perch/occlusion
  (`app/src/claudlet/platform/geom/macos.py`). The installer (`pipx install claudlet` or the
