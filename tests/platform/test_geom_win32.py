...
def test_to_logical_identity_at_100_percent():
    ...
    assert windows_win32._to_logical(100, 200, 800, 600, 1.0) == (100, 200, 800, 600)


def test_to_logical_divides_by_scale():
    # 150% display: a physical rect scales down into Qt logical space
    assert windows_win32._to_logical(150, 150, 1200, 900, 1.5) == (100, 100, 800, 600)
    # 125%
    assert windows_win32._to_logical(125, 0, 500, 250, 1.25) == (100, 0, 400, 200)
...
def test_dpi_scale_defaults_to_1_without_symbol(monkeypatch):
    ...
    monkeypatch.setattr(windows_win32, "_HAS_GETDPI", False)
    assert windows_win32._dpi_scale(1234) == 1.0
...
def test_find_window_by_class_matches_substring(monkeypatch):
    ...
    monkeypatch.setattr(windows_win32, "user32", object())   # non-None gate
    monkeypatch.setattr(windows_win32, "_enum_windows", lambda exclude_hwnd=None: rows)
    assert windows_win32.find_window_by_class(["cascadia"]) == 22
    assert windows_win32.find_window_by_class(["nope"]) is None


def test_find_window_by_class_noop_without_user32(monkeypatch):
    monkeypatch.setattr(windows_win32, "user32", None)
    assert windows_win32.find_window_by_class(["x"]) is None


def test_user_locale_empty_without_kernel32(monkeypatch):
    monkeypatch.setattr(windows_win32, "kernel32", None)
    assert windows_win32.user_locale() == ""
